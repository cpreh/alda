...
void serialize(alda::serialization::ostream &_stream, alda::message::base<TypeEnum> const &_message)
{
  static_assert(std::is_unsigned_v<LengthType>);

  alda::raw::buffer const buffer(_message.to_buffer());

  alda::serialization::length::put<TypeEnum, LengthType>(_stream, buffer.size());

  alda::serialization::write_id(_stream, _message);

  alda::serialization::buffer_to_stream(_stream, buffer);
}
