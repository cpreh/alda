...
alda::message::optional_base_unique_ptr<TypeEnum> deserialize(
    ...
    alda::serialization::length::remaining_size_function const &_remaining_size)
{
  ...
  using result_type = alda::message::optional_base_unique_ptr<TypeEnum>;

  return fcppt::optional::bind(
      alda::serialization::length::extract<LengthType>(_stream, _remaining_size),
      [&_context, &_stream, &_remaining_size](LengthType const _length)
      {
        if (_remaining_size() < fcppt::cast::size<std::streamsize>(fcppt::cast::to_signed(_length)))
        {
          fcppt::algorithm::repeat(sizeof(LengthType), [&_stream] { _stream.unget(); });

          return result_type();
        }

        return result_type{alda::serialization::deserialize(_context, _stream)};
      });
}
