...
public:
  using type_enum = typename TypeEnum::type;

  using unique_ptr = typename alda::message::base_unique_ptr<TypeEnum>;

  virtual ~base() = 0;

  [[nodiscard]] virtual type_enum type() const = 0;

  [[nodiscard]] virtual alda::raw::size_type size() const = 0;

  [[nodiscard]] virtual alda::raw::buffer to_buffer() const = 0;

  [[nodiscard]] virtual unique_ptr clone() const = 0;
};
