...
inline auto bind(Type &&_value, Function const &_function)
    -> std::enable_if_t<!Stream::can_fail, decltype(_function(std::forward<Type>(_value)))>
{
...
bind(fcppt::either::object<alda::raw::stream::error, Type> &&_value, Function const &_function)
    -> std::
        enable_if_t<Stream::can_fail, decltype(fcppt::either::bind(std::move(_value), _function))>
{
