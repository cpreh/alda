...
void place(
    alda::raw::dispatch_type<alda::bindings::variant<Types, AdaptedTypes>>,
    alda::raw::element_type<alda::bindings::variant<Types, AdaptedTypes>> const &_value,
    alda::raw::pointer _mem)
{
  using binding = alda::bindings::variant<Types, AdaptedTypes>;
  ...
  _mem = alda::raw::place_and_update<index_type>(index, _mem);

  fcppt::variant::apply(
      [_mem](auto const &_type)
      {
        alda::raw::place<fcppt::mpl::list::at<
            AdaptedTypes,
            fcppt::mpl::list::index_of<Types, std::decay_t<decltype(_type)>>>>(_type, _mem);
      },
      _value);
}

template <typename Stream, typename Types, typename AdaptedTypes>
alda::raw::stream::result<Stream, alda::bindings::variant<Types, AdaptedTypes>> make_generic(
    alda::raw::dispatch_type<alda::bindings::variant<Types, AdaptedTypes>>,
    ...
    alda::raw::stream::reference<Stream> _stream)
{
  ...
  return alda::raw::stream::bind<Stream>(
      alda::raw::make_generic<Stream, index_type>(_stream),
      [&_stream](alda::raw::element_type<index_type> const _index)
      {
        return fcppt::mpl::list::invoke_on<Types>(
            _index,
            [&_stream](auto const _tag)
            {
              FCPPT_USE(_tag);

              using adapted_type = fcppt::mpl::list::at<
                  AdaptedTypes,
                  fcppt::mpl::list::index_of<Types, fcppt::tag_type<decltype(_tag)>>>;

              return alda::raw::stream::bind<Stream>(
                  alda::raw::make_generic<Stream, adapted_type>(_stream),
                  [](alda::raw::element_type<adapted_type> &&_inner)
                  {
                    return alda::raw::stream::return_<Stream>(
                        fcppt::variant::from_list<Types>(std::move(_inner)));
                  });
            },
            [_index]
            {
    ...
    alda::raw::element_type<alda::bindings::variant<Types, AdaptedTypes>> const &_value)
{
  ...
  return alda::raw::needed_size<index_type>(index) +
         fcppt::variant::apply(
             [](auto const &_type)
             {
               return alda::raw::needed_size<fcppt::mpl::list::at<
                   AdaptedTypes,
                   fcppt::mpl::list::index_of<Types, std::decay_t<decltype(_type)>>>>(_type);
             },
             _value);
}
